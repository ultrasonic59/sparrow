...
*(udef_addr+addr+1)=(dat>>16)&0xffff;
///spi1_mutex_unlock();
}
...
for(cnt_ready=0;cnt_ready<MAX_READY_CNT;cnt_ready++)
	...
	if((t_dat&SPI_BUSY_BIT)==0)
		break;
		printk( KERN_ERR " \n\r [mod]wait_ready[%x][%x]",t_dat ,cnt_ready);
	vdelay(4000000);
...
{
int rdy;
wr_dat_fpga(ADDR_CNF_SPI,SPI_NOINV_SCK);///
...
int mod_get_spi_da(u8 iaddr,u16 *odata )
{
int rdy;
u16 rez;
u16 _tst;
uint8_t t_addr;
///printk( KERN_ERR "\n [mod]mod_get_spi_da[%x]===",iaddr );
wr_dat_fpga(ADDR_CNF_SPI,SPI_NOINV_SCK);///
...
///printk( KERN_ERR "\n x [mod]mod_get_spi_da[%x]===",iaddr );
wr_dat_fpga(ADDR_SPI_CS_A,0x1);
t_addr=iaddr|(0x1<<7);///read
wr_dat_fpga(ADDR_SPI_WR_DATA,t_addr);///set addr
rdy=wait_ready_spi_adda();
if(rdy!=0)
	{
	wr_dat_fpga(ADDR_SPI_CS_A,0x0);
...
///printk( KERN_ERR "\n xxx [mod]mod_get_spi_da[%x]===",iaddr );
...
////printk( KERN_ERR "\n xxxx [mod]mod_get_spi_da[%x]===",i_addr );
_tst=rd_dat_fpga(ADDR_SPI_RD_DATA);
rez|= _tst&0xff;

wr_dat_fpga(ADDR_SPI_CS_A,0x0);
///printk( KERN_ERR "\n xxxx [mod]mod_get_spi_da[%x:%x[%d]]===",iaddr,rez,rdy );
...
////			printk( KERN_INFO "error : %d\n" ,-EFAULT);  //
		...
		{
////		printk( KERN_ERR "\nmod_IOCTL_GET_ALT_DATA ");  //
		get_spi_da_data(&dac_spi_dat_req);
		if( copy_to_user( (void*)arg, &dac_spi_dat_req, _IOC_SIZE( cmd ) ) )
			{
////			printk( KERN_INFO "error : %d\n" ,-EFAULT);  //
			return -EFAULT;
			}
