...
////	        return 1;
	   ...
	   {
////	     printk(KERN_ALERT " GPIO number %d ok.\n", RST_DAC_PIN);
    ret = gpio_request(RST_ADC_PIN,"ADC_RST");
    if (ret)
    {
        printk(KERN_ALERT "Gpio request failed.\n");
////        return -2;
    	...
    	{
    	rst_adc.gpio=RST_ADC_PIN;
    	rst_adc.label = "ADC_RST";
        gpio_direction_output(rst_adc.gpio, 0);
        gpio_set_value(rst_adc.gpio, 0);
