...
///==================
...
#if 1
	{
		adc_spi_req_t t_adc_da;
		num_bytes= (*(bbuf+0))|(*(bbuf+1)<<8);
		n_bytes=MIN(num_bytes,sizeof(adc_spi_req_t));
		memcpy(&t_adc_da,bbuf+2,n_bytes);
		fprintf(stderr,"\nNNUM_PUT_SPI_ADC_DAT[%x:%x]",t_adc_da.addr,t_adc_da.data);
		ioctl( mod_fd, IOCTL_PUT_SPI_AD, &t_adc_da );
	}
#endif
		break;
#if 1
	case NUM_SEND_SPI_ADC_REQ:
		...
		num_bytes= (*(bbuf+0))|(*(bbuf+1)<<8);
		n_bytes=MIN(num_bytes,sizeof(adc_spi_req_t));
		memcpy(&g_adc_dat,bbuf+2,n_bytes);
		fprintf(stderr,"\nNUM_SEND_SPI_ADC_REQ[%x]",g_adc_dat.addr);
		ioctl( mod_fd, IOCTL_SET_SPI_AD_REQ, &g_adc_dat );
		fprintf(stderr,"\nNUM_SEND_SPI_ADC_REQ1[%x:%x]",g_adc_dat.addr,g_adc_dat.data);
		}
///		get_req_spi_adc(&g_adc_spi_dat);
		break;
#endif
...
volatile uint16_t tst_t=0;

void set_tst_oscs(uint16_t idat)
{
int ii;
...
///				,t_alt_dat_req.ndata,t_alt_dat_req.data[0]);
	...
	case NUM_GET_SPI_ADC_DAT:
		...
		if( ioctl( mod_fd, IOCTL_GET_SPI_AD_DATA, (void*)&t_adc_spi_req ) >=0)
			{
			memcpy(buf,&t_adc_spi_req,sizeof(adc_spi_req_t));
			rez=sizeof(adc_spi_req_t);
			fprintf(stderr,"\nNUM_GET_SPI_ADC_DAT[%x:%x]",t_adc_spi_req.addr
				,t_adc_spi_req.data);
			}
	else
		{
...
if(tst_t>=255)
	tst_t=0;
#endif
		memcpy(buf, buffOscs,LEN_OSC*sizeof(uint16_t));
		rez=LEN_OSC*sizeof(uint16_t);
////		flg_req_osc=1;
////		fprintf(stderr,"<");
		break;
#if 0
...
////		flg_req_osc=1;
	...
	{
	uint16_t tmp1=curr_ad_dat;
	memcpy(buf,&tmp1,sizeof(uint16_t));
	rez=sizeof(uint16_t);
